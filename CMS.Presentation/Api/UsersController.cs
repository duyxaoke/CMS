...
using CMS.Presentation.Controllers.Api;
...
using static CMS.Presentation.Models.RoleClaimsViewModel;
    ...
    public class UsersController : ApiControllerBase
    {
        ...
        public async Task<IHttpActionResult> GetByUsernameAsync(string userid)
        {
            try
            {
                ...
                if (result == null)
                    return NotFound();
                return Ok(new { Name = result.Name, Email = result.Email, UserName = result.UserName, Balance = result.Balance });
            }
            catch (Exception ex)
            {
                return InternalServerError();
