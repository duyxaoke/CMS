...
namespace CMS.Presentation.Controllers.Api
{
    ...
    public class LanguagesController : ApiControllerBase
    {
        ...
        public LanguagesController(ILanguageAppService service)
        {
            _service = service;
        }

        [HttpGet]
        [ResponseType(typeof(IEnumerable<LanguageViewModel>))]
        ...
        public IHttpActionResult Post([FromBody]LanguageViewModel model)
        {
            ...
            {
                _service.Add(model);
                return Content(HttpStatusCode.Created, model.LanguageId);
            }
            catch (DbUpdateException ex)
            {
                if (Exists(model.LanguageId))
                {
                    return Conflict();
                }
                else
        ...
        [ResponseType(typeof(void))]
        [EnableThrottling(PerSecond = 1)]
        public IHttpActionResult Put([FromBody]LanguageViewModel model)
        {
            if (!ModelState.IsValid)
            {
                var errors = new List<string>();
